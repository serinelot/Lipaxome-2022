setwd("C:/Users/serin/OneDrive - USherbrooke/1- UdeS/1-Maîtrise/LipaXome/Dataset Geo_Omnibus/GSE64018/data")
...
# fmr1.string <- read.csv("C:/Users/serin/OneDrive - USherbrooke/1- UdeS/1-Maîtrise/RNA_chol_gene_asd/Set de gènes/FMRP_molecular_partners_PMID_30905341.csv"
#                         , header = TRUE)
...
ggplot(data=pca_fmr1.data, aes(x = X, y = Y, label = Sample, color = factor(Sample))) +
  ...
  theme_bw() +
  ggtitle("Fmr1 PCA Graph - GSE64018")
...
ggplot(data=pca_chol.data, aes(x = X, y = Y, label = Sample, color = factor(Sample))) +
  ...
  theme_bw() +
  ggtitle("Cholesterol PCA Graph - GSE64018")
...
ggplot(data=top_10_genes_chol_pca, aes(x = reorder(Genes, -Variation), y = Variation)) + 
  ...
  coord_flip() +
  ggtitle("Top 10 genes associated with cholesterol influencing the PCA graph - GSE64018") +
  labs(x = "Genes", y = "Score")


### Set de gènes FMR1 + chol (codant du pc portable) ###
...
idx <- tempdf$SYMBOL %in% fmr1.chol.string$Gene.symbol
table(idx)

data.fmr1.chol <- data_log2[idx,]                    
dim(data.fmr1.chol)

row.names(data.fmr1.chol) <- tempdf$SYMBOL[which(idx)]

##### Exporter excel fmr1

write.xlsx(data.fmr1.chol,
           file="GSE64018_fmr1_chol.xlsx",
           sheetName = "1",
           append = FALSE)
